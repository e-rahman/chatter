...
var file = new File(currentFolder+"/coral_reef_type.xml");
var file2 = new File(currentFolder+"/coral_reef_values.xml");

var coral_reef_typeXML, coral_reef_valuesXML;

// Call function to read coral reef data in two XML files
readcoral_reef_typeXMLFile(file);
readcoral_reef_valuesXMLFile(file2);
...
for(i = 0; i < 10; i++){
    for(j = 0; j < 10; j++){
        // Get the emotion value in the XML element associated with the current loop index
            var emotion_value = coral_reef_valuesXML.data[indexCounter].value;
        ...
        // Get the emotion value in the XML element associated with the current loop index (indexCounter)
            var emotion_value = coral_reef_typeXML.data[indexCounter].value;
