...
var file = new File(currentFolder+"/top_gun_type.xml");
var file2 = new File(currentFolder+"/top_gun_values.xml");

var top_gun_typeXML, top_gun_valuesXML;

// Call function to read Top Gun data
readtop_gun_typeXMLFile(file);
readtop_gun_valuesXMLFile(file2);
...
for(i = 0; i < 10; i++){
    for(j = 0; j < 10; j++){
        // Get the emotion value in the XML element associated with the current loop index
            var emotion_value = top_gun_values.data[indexCounter].value;
        ...
        // Get the emotion value in the XML element associated with the current loop index (indexCounter)
            var emotion_value = top_gun_type.data[indexCounter].value;
